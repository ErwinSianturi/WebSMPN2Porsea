...
<body>
    ...
    <div class="jumbotron jumbotron-fluid position-relative" style="margin-bottom: 90px; background-image: url('{{ asset('assets/PAGARDEPANSEKOLAH.jpg') }}'); background-size: cover; background-position: center; background-repeat: no-repeat;">
        ...
        <div class="container text-center my-5 py-5 position-relative">
            <h1 class="text-white display-1 mb-1 font-weight-bold text-shadow text-uppercase">Berita</h1>
            ...
            <div class="row">
                <div class="col-lg-5 mb-5 mb-lg-0" style="min-height: 300px;">
                    <div class="position-relative h-100">
                        <img class="position-absolute w-100 h-100 img-fluid" src="{{ asset('foto/' . $item->foto) }}" style="object-fit: cover;" data-toggle="modal" data-target="#modal{{$item->id}}">
                    </div>
                </div>
                <div class="col-lg-7  semua-jus">
                    <div class="section-title position-relative mb-4">
                        <h1 class="display-4 text-primary" data-toggle="modal" data-target="#modal{{$item->id}}">{{ $item->judul }}</h1>
                    </div>
                    <p class="mb-4 pb-2 text-justify">{!! substr($item->isi, 0, 200) !!}...</p>
                    <a href="{{ route('show', ['id' => $item->id]) }}" class="btn btn-primary">Baca Selengkapnya</a>
    ...
    <center>
        <div class="section-title text-center position-relative mb-5">
            <h6 class="d-inline-block position-relative text-secondary text-uppercase pb-2">Info PSB </h6>
        </div>
    </center>
    ...
    <div class="container-fluid py-5">
        <div class="container py-5">
            <div class="row">
                ...
                <div class="col-lg-7  semua-jus">
                    <div class="section-title position-relative mb-4">
                        <h1 class="display-4" data-toggle="modal" data-target="#modal{{$item->id}}">{{ $item->judul }}</h1>
                    </div>
                    <p class="mb-4 pb-2">{!! substr($item->isi, 0, 200) !!}...</p>
                    <a href="{{ route('show', ['id' => $item->id]) }}" class="btn btn-primary">Baca Selengkapnya</a>
